...
import { colors, fontFamily, lessonImages, placeholderImages } from '@/theme';
...
export default function LearnScreen() {
  ...
  const heroSource = learnData.activeLesson.title.toLowerCase().includes('café')
    ? { uri: placeholderImages.cafeHero }
    : (lessonImages[learnData.activeLesson.imageKey] ?? { uri: placeholderImages.cafeHero });
    ...
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      ...
      >
        <View style={styles.headerRow}>
          <Pressable onPress={() => router.push('/')} hitSlop={12} style={styles.backButton}>
            <Text style={styles.backIcon}>←</Text>
          </Pressable>

          <View style={styles.headerCopy}>
            <Text style={styles.headerTitle}>{learnData.activeLesson.title}</Text>
            <Text style={styles.headerSubtitle}>
              Unit {learnData.unit.order} • {learnData.completedCount + 1} / {learnData.totalCount}{' '}
              lessons
            </Text>
          </View>
        ...
        <View style={styles.body}>
          <LearnSegmentedControl activeTab={activeTab} onChange={setActiveTab} />
